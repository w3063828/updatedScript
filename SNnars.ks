...
start().
   ...
   until SHIP:ALTITUDE < 30000 and ship:AIRSPEED < 150 {
    set steering to prograde * r(-45, 0, 0):vector.
    lock throttle to 0.
   ...
   LOCK STEERING TO HEADING(TARGET:HEADING,desiredpitch).
    until dtg() > (stoppingDistance + 100) AND VB() < 3000 {
       if vb() > 5000 {set PIDpitch to pidloop(1, 0.2, 0.01, -10, 10).}
