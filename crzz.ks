...
until ship:parts:length < original_length { wait 0.2.}
//stage.
...
until ship:parts:length > original_length {
   set vbold to vb().
  IF VB() < 1  {Set talt to (talt-0.33). }
  if vb() < 30 AND VB() > 1 {lock steering to up. LOCK TALT TO MATHgg().  }
...
FUNCTION start {
 ...
 SET gp to SHIP:BODY:GEOPOSITIONOF(gp:POSITION).
  lock grav to body:mu / body:position:sqrmagnitude.
  gear off.
  set leg_part to ship:partstagged("hdf")[0].
...
FUNCTION shutdown_stack {
 ...
 FUNCTION mathGG {
  lock mathResult to min((height +5),((height  + (VB * 4)) - 0)) .
  RETURN mathResult.
} 
function dtg {
