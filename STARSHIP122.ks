...
until runmode = 0 { //Run until we end the program
    ...
    if runmode = 1 { 
       until SHIP:ALTITUDE < 25000 and ship:AIRSPEED < 1000 {
       ...
       print "GROUND SPEED:" + round(SHIP:GROUNDSPEED,1) + "      " at (5,14).
        ...
        if SHIP:ALTITUDE < 15000 and ship:AIRSPEED < 1000 {set runmode to 2.}
    }
    ...
    if runmode = 2 { 
       ...
       if impactTime > 165 {set PIDpitch to pidloop(4, 0.1, 0.1, -35, 35).}
   ...
   if runmode = 4 {  
       ...
       clearscreen.
        until ship:status = "Landed" {
         ...
         if dtg()< 150 and vb()< 55 {set talt to min(5,talt - 0.08).}
