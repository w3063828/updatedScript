...
clearscreen.
  ...
  rcs on.
  set r1 to 0.
  lock r1 to max(-135,-90 +((speedlimit - groundspeed)*9)).
  local original_length is ship:parts:length. 
  until ship:parts:length < original_length {
    
  if Ship:groundspeed > speedlimit { LOCK STEERING TO LOOKDIRUP(ANGLEAXIS(r1,
                  VCRS(VELOCITY:SURFACE,BODY:POSITION))*VELOCITY:SURFACE,
           FACING:TOPVECTOR).} 
  if Ship:groundspeed < (speedlimit - 0.25 ) { lock steering to lookdirup(up:vector * 10 + RV, facing:topvector).}
  set PID:setpoint to math().
  if VB < 3 { SET tAlt to (talt - 1).}
   ...
   set target_twr to pid:update(time:seconds, ship:verticalspeed).
    ...
    IF ABORT {UNLOCK STEERING. UNLOCK THROTTLE.}
    wait 0.2.
    }
shutdown_stack().
sas on.
...
function GS{ 
    ...
    local A IS body:geopositionOf(ship:position).
...
function AC {return min((talt),talt + (VB*5)).}
...
function GSD{ 
 ...
 if abs(sq16a) < abs(sq1a) and abs(sq16a) < abs(sq3a) and abs(sq16a) < abs(sq4a) and abs(sq16a) < abs(sq5a) and abs(sq16a) < abs(sq6a) and abs(sq16a) < abs(sq7a) and abs(sq16a) < abs(sq8a) and abs(sq16a) < abs(sq9a) and abs(sq16a) < abs(sq10a) and abs(sq16a) < abs(sq11a) and abs(sq16a) < abs(sq12a) and abs(sq16a) < abs(sq13a) and abs(sq16a) < abs(sq14a) and abs(sq16a) < abs(sq15a) and abs(sq16a) < abs(sq2a)  { set best_scorep to abs(sq16a).}
    ...
    print best_score at (5,5).
    print "gsdF" at (5,6).
     print round(scoretobeat,2) at (5,10).
     print round(x_f,3) at (5,7).
     print round(x_f1,3) at (5,8).
      print tmizz at (5,9).
    //wait 2.
