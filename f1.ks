...
function HH {
  lock maxtwr to (ship:maxthrust * 0.93)/ (GRAV1 * ship:mass).
  global pid is pidloop(2.72727272727273, 2.36900403330529, 0.226531243370937, 0, maxtwr).
  set tAlt to 100.
  lock steering to descent_vector().
  clearscreen.
  if altitude < 5000 and ship:AIRSPEED > 100 { set runmode to 20.}
  ...
  until runmode = 0 { //Run until we end the program
    ...
    if runmode = 2 {
      ...
      until ship:status = "Landed" {
        ...
        set talt to max(1,talt - 0.35).
        set target_twr to pid:update(time:seconds, dtg()).
        lock throttle to min(target_twr / maxtwr, 1).
      print "ground distance:    " + round(dtg()) + "      " at (5,4).
      print "ground speed:    " + round(ship:GROUNDSPEED) + "      " at (5,5).
      print "vertical speed:    " + round(ship:verticalSpeed) + "      " at (5,6).
      ...
      print "RUNMODE:    " + runmode + "      " at (5,3).
      print "stopD    " + stoppingDistance() + "      " at (5,10).
      if SAS { SAS off. RUNPATH("0:/cr.KS"). }
      wait 0.35.
      set runmode to 2.
      //TOGGLE ag4.
        //set runmode to 4. }
...
if runmode = 20 {
   until AIRSPEED < 200{
      print "air speed:    " + round(ship:airSPEED) + "      " at (5,5).
    lock throttle to (2* Ship:Mass * GRAV / ship:availablethrust).
    wait 0.
  }
  if AIRSPEED < 200 {set runmode to 1.}}
 }
}
