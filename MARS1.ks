...
function main {
  ...
  until apoapsis > 100000 {
    ...
    print "LF1  " + round(LF1:amount) + "      " at (5,11).
    if LF:amount < 7200{
      ag6 on.}
    if LF1:amount < 7200{
       runpath("0:/cir.ks").}}
...
function doTransfer {
  ...
  wait until body = duna.
  wait 1.
}

function angleToDuna {
  parameter t.
  return vectorAngle(
    Kerbin:position - positionAt(ship, t),
    Kerbin:position - positionAt(Duna, t)
  ).
}
...
function munTransferScore {
  ...
  if mnv:orbit:hasNextPatch {
    ...
    set result to distanceToDunaAtApoapsis(mnv).
  }
  removeManeuverFromFlightPlan(mnv).
  return result.
}

function distanceToDunaAtApoapsis {
  parameter mnv.
  ...
  return (positionAt(ship, apoapsisTime) - positionAt(Duna, apoapsisTime)):mag.
}
...
function doAscent {
  ...
  SET Sidetank TO SHIP:PARTSDUBBED("fueltank0")[0].
  FOR res IN Sidetank:RESOURCES {
    IF res:NAME = "LIQUIDFUEL" {
            lock LF TO res.
            BREAK.
    }}
SET Sidetank TO SHIP:PARTSDUBBED("fueltank1")[0].
  FOR res IN Sidetank:RESOURCES {
    IF res:NAME = "LIQUIDFUEL" {
            lock LF1 TO res.
            BREAK.
