...
//runpath("0:/f2.ks").
 if runmode = 3 { 
  set PIDfore:SETPOINT TO mathA[0].
  set PIDstar:SETPOINT TO mathA[1].
  set PIDtop:SETPOINT TO mathA[2].
  until vb < 10 { 
  set talt to 20. 
  lock steering to lookdirup(up:vector * 10 + RV, facing:topvector).
...
clearscreen.
  ...
  lock throttle to (GRAV / maxAcc + (talt2 - min(talt2 + 30,apoapsis)) / 100 - verticalspeed / 50) / vdot(up:vector,facing:vector).
   print "talt:    " + talt + "      " at (5,4).
    ...
    print "myVelocity:  " + axis_speed[0] + "      " at (5,03).
  ...
  set TRTM to VECDRAWARGS( V(0,0,0),target:position,green,"ship",1,true).
   wait 0.
    IF vb < 0.06 {Lock tAlt to MIN(20,(vb * 300)).}
   }
}
if runmode = 5 { runpath("0:/gnu.ks").}
}
...
function AC {return min(20,vb * 300).}
...
FUNCTION axis_distance {
	...
	LOCAL dist IS vb().
	LOCAL distFor IS VDOT(distVec,ship:Facing:topVECTOR).	//if positive then stationPort is ahead of ship:Port, if negative than stationPort is behind of ship:Port
	...
	LOCAL disttop IS VDOT(distVec,ship:Facing:FOREVECTOR).
  RETURN LIST(dist,distFor,distStar,disttop).}
  
  function mathA {
  LOCAL sF IS (axis_distance[1] / 10).	//if positive then stationPort is ahead of ship:Port, if negative than stationPort is behind of ship:Port
	LOCAL sS IS (axis_distance[2] / 10).	//if positive then stationPort is to the right of ship:Port, if negative than stationPort is to the left of ship:Port
	LOCAL sT IS (axis_distance[3] / 10).
  RETURN LIST(sF,sS,sT).}
