...
if best_score < scoreToBeat {
  ...
  gsd(). //runs normal loop as long a score is best score continues to be lower than score to beat.
      ...
      set sign to (-1)^(theta).
      set RADIUS to (RADIUS + 0.241).
      set x_f to  sign * SQRT(RADIUS) * sin(SQRT(RADIUS)).
      set x_f1 to sign * SQRT(RADIUS) * COS(SQRT(RADIUS)).
...
function GSD{ 
     ...
     local y IS body:geopositionOf(center + 10 * north:vector - 10 * east).  
        ...
        local y_vec is y:altitudeposition(y:terrainheight).
            ...
            local sq1 is vcrs(a_vec - f_vec, b_vec - g_vec).
            local sq2 is vcrs(b_vec - g_vec, c_vec - h_vec).
            local sq3 is vcrs(c_vec - h_vec, d_vec - i_vec).
            local sq4 is vcrs(d_vec - i_vec, e_vec - j_vec).

            local sq5 is vcrs(f_vec - k_vec, g_vec - l_vec).
            local sq6 is vcrs(g_vec - l_vec, h_vec - m_vec).
            local sq7 is vcrs(h_vec - m_vec, i_vec - n_vec).
            local sq8 is vcrs(i_vec - n_vec, j_vec - o_vec).

            local sq9 is vcrs(k_vec - p_vec, l_vec - q_vec).
            local sq10 is vcrs(l_vec - q_vec, m_vec - r_vec).
            local sq11 is vcrs(m_vec - r_vec, n_vec - s_vec).
            local sq12 is vcrs(n_vec - s_vec, o_vec - t_vec).

            local sq13 is vcrs(p_vec - u_vec, q_vec - v_vec).
            local sq14 is vcrs(q_vec - v_vec, r_vec - w_vec).
            local sq15 is vcrs(r_vec - w_vec, s_vec - x_vec).
            local sq16 is vcrs(s_vec - x_vec, t_vec - y_vec).
