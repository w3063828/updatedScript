...
FUNCTION start {
  ...
  lock TARGET TO VESSEL("sBN 1.35").
 ...
 //  set height to body:geopositionOf(gp:position):terrainHeight.
  ...
  gear off.
  set leg_part to ship:partstagged("hd")[0].
...
FUNCTION shutdown_stack {
 ...
 FUNCTION mathGG {
  lock mathResult to min((height +5),((height  + (VB * 4)) - 0.14)) .
  RETURN mathResult.
} 
function dtg {
...
FUNCTION math {
  lock mathResult to ((30 - ship:groundspeed)/2.5).
