...
until ship:status = "Landed" {         
   set vbold to vb().
  IF VB() < 0.35  {Set talt to (talt-0.08). }
  if vb() < 5 AND VB() > 1 {lock steering to up. }
  if vb() > 5 {set STEERING TO HEADING(gp:HEADING,math()).}
   set PIDfore:SETPOINT TO MIN(7,(axis_distance[1]/12)).
   set PIDstar:SETPOINT TO MIN(7,(axis_distance[2]/12)).
   set PIDtop:SETPOINT TO MIN(7,(axis_distance[3]/12)).
...
FUNCTION STARTA {
  ...
  WAIT UNTIL (steering_alinged_duration() >= 1) OR ABORT.
  set STEERING TO HEADING(GP:HEADING,math()).
}
...
FUNCTION math {
  lock mathResult to ((10- ship:groundspeed)/2.5).
