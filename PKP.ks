...
until ship:status = "Landed" and vb() < 0.5 and dtg < 2 {
   set vbold to vb().
  if vb() < 30 {lock steering to up.  }
   if vb() < 10 {lock talt to mathgg().}
...
FUNCTION shutdown_stack {
 ...
 FUNCTION mathGG {
  lock mathResult to min(20,((VB * 20)-1)) .
  RETURN mathResult.
} 
function dtg {
 ...
 local bounds_box is leg_part:bounds.
   return min(bounds_box:bottomaltradar,betterALTRADAR).
}
