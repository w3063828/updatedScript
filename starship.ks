...
until runmode = 0 { //Run until we end the program
    ...
    if runmode = 21 { 
        ...
        if SHIP:ALTITUDE < 70000 and ship:AIRSPEED > 1120 {
                set STEERING to RETROGRADE.
                lock throttle to (4 * Ship:Mass * GRAV/ ship:availablethrust).
               
                }

         if SHIP:ALTITUDE < 70000 and ship:AIRSPEED < 1115{
                lock throttle to 0.
                set runmode to 22.}}
        

    if runmode = 22 { //bellyflop
     LOCK STEERING TO LOOKDIRUP(ANGLEAXIS(-45,
                  VCRS(VELOCITY:SURFACE,BODY:POSITION))*VELOCITY:SURFACE,
           FACING:TOPVECTOR).
        
        if SHIP:ALTITUDE < 30000 and ship:AIRSPEED > 1060 {
                wait 0.
                   }

         if SHIP:ALTITUDE < 30000 and ship:AIRSPEED < 1100 {
                lock throttle to 0.
                set runmode to 23.}}
