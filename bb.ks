...
set beta to false.
  until beta = true { 
          set posError to target_gp:position - addons:tr:impactpos:position.
            ...
            print "vel " + round(ship:groundspeed ,2) + "      " at (5,5).
            print "Desired lateral speed " + round(dl(),3) + "      " at (5,6). 
            print "sign " + round(ob,3) + "      " at (5,7).
            
            set dlold to dl().
            if ob < (0.1){ set beta to true. break.}
            wait 0.01.
            }
//CLEARSCREEN.
//set throttle to 0.
...
brakes on.
  until ship:status = "Landed" {     
   ...
   set target_twr to pid:update(time:seconds, ship:verticalspeed).
      if TTI < 15 { lock throttle to min(target_twr / maxtwr, 1).
          SET PID:SETPOINT TO math().}
      if dtg() < 1000 and ship:verticalspeed > -35 {set steering to up.  }
...
FUNCTION start {
  ...
  LOCK STEERING TO HEADING(gp:HEADING,20).
  rcs on.
  wait 10.
  ...
  lock ob to (dlold - dl()).
   AG9 ON. 
  set throttle to (1 * Ship:Mass * GRAV / ship:availablethrust).  
  wait 5.
  set throttle to 0.65.
  
}

function AC {
  
  return round(dlold - dl(),3).
  }
...
FUNCTION math {  
  lock mathResult to ((talt-dtg())/7.5).
  RETURN mathResult.
}
function VB{
   return vxcl(up:vector,gp:POSITION):mag.
