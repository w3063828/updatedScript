...
LOCK grav1 TO SHIP:SENSORS:grav:mag.
 SET Sidetank TO SHIP:PARTSDUBBED("fueltank")[0].
  ...
  SET idetank TO SHIP:PARTSDUBBED("ueltank")[0].
    FOR res IN idetank:RESOURCES {
      IF res:NAME = "LIQUIDFUEL" {
              lock LF1 TO res.
             BREAK.
   }}
...
function h_s {
 ...
 if runmode = 1 { 
   ...
   if SHIP:ALTITUDE < 30000 and ship:GROUNDSPEED > 10 {
...
function perform_suicide_burn {
  ...
  lock throttle to t.
  until radar() < SUICIDE_DISTANCE_MARGIN {
...
function has_impact_time {
  parameter margin.
  local a is grav1 * (1 - availtwr()).
  local v is -verticalspeed.
  local d is radar() - margin.
