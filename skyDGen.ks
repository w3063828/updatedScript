...
until runmode = 0 { //Run until we end the program
    ...
    if runmode = 1 { 
       until SHIP:ALTITUDE < 30000 and SHIP:GROUNDSPEED < 350{
       ...
       print "GROUND SPEED:" + round(SHIP:GROUNDSPEED,1) + "      " at (5,14).
        ...
        if SHIP:ALTITUDE < 30000 and SHIP:GROUNDSPEED < 350 {set runmode to 2.}
    }
    ...
    if runmode = 2 { 
       ...
       AG1 ON.
       AG2 ON. 
       //set gp to Ship:Body:GeoPositionOf( Ship:Position+17000 * Ship:Facing:ForeVector ).
      ...
      print "stopD    " + stoppingDistance() + "      " at (5,21).
       ...
       IF dtg() < 1550 {set runmode to 3.}
    }
        
    if runmode = 3 { // Land on the ground
       toggle ag3.
       lock steering to LOOKDIRUP(TRTM2,SHIP:FACING:topVECTOR).
   ...
   if runmode = 4 {  
       ...
       ag10 on.
        until ship:status = "Landed" {
         ...
         if dtg()< 101 and vb()< 5050 {set talt to max(5,talt - 0.6).
         ag9 on.}
         SET PID:SETPOINT TO math().
         set target_twr to pid:update(kkk,ship:AIRSPEED).
         lock throttle to min(target_twr / maxtwr, 1).
